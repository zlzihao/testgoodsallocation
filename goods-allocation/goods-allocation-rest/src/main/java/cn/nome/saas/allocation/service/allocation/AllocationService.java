...
import cn.nome.saas.allocation.service.rule.FlowRuleEngineService;
import cn.nome.saas.allocation.service.rule.ForbiddenRuleService;
import cn.nome.saas.allocation.service.rule.RequireRuleEngineService;
...
public class AllocationService {
    ...
    public void runAllocationTask(int taskId) {
        Task task = null;
        ...
        if (taskDO != null) {
            ...
            List<Task> taskDOList = taskService.getNeedRunTaskList(Constant.COLLECT_ALLOCATION_TYPE);

            if (CollectionUtils.isNotEmpty(taskDOList)) {
                long runningTaskCount = taskDOList.stream().filter(t -> t.getTaskStatus() == 2).count();
                if (runningTaskCount == 0) {
                    //  取第一个任务开始执行
                    task = taskDOList.stream().filter(t -> t.getTaskStatus() == 1).findFirst().orElse(null);
                }
            }
        }
    ...
    public void allocation(Task task) {
        ...
        //Set<String> matCodeSet = demandList.stream().map(OutOfStockGoodsDO::getMatCode).collect(Collectors.toSet());
    ...
    private void calcBestQty(Task task,int safeDays,Set<String> shopIdList,List<Future> calcIdealQtyFutureList) {
        ...
        List<OutOfStockGoodsDO> inStockShopList = allocationExtraDataMapper.getInStockBetweenSeven(shopIdList); // 7天有调入的
    ...
    public Integer calcBestAllocationMatch(Task task,List<OutOfStockGoodsDO> demandList,List<OutOfStockGoodsDO> supplyList) {
        ...
        while (true) {
            ...
            AllocationShop bestMatchSupplyShop = flowRuleEngineService.calcSupplyScore(task,amountDemandList,supplyList,highRateRejectShop);
